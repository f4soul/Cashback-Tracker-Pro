...
import { FileText, FileSpreadsheet, Image as ImageIcon, ArrowUpDown } from 'lucide-react';
...
export const CashbackTable: React.FC<CashbackTableProps> = memo(({ 
  ...
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>(() => {
    return (localStorage.getItem('table_sort_order') as 'asc' | 'desc') || 'asc';
  });
  ...
  return (
    <div id={id} className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 p-2 sm:p-4 w-full mx-auto overflow-hidden relative transition-all duration-500">
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4 px-1">
          <div className="flex-1 text-left">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight whitespace-nowrap flex items-center justify-start gap-1">
              ...
              >
                <ArrowUpDown className={clsx("w-3.5 h-3.5", sortOrder === 'desc' ? "text-[var(--accent-color)]" : "text-gray-400")} />
              </button>
            </h2>
          </div>
        ...
        <div className="flex flex-col gap-1">
          {entries.map((entry) => {
            ...
            const sortedCategories = sortOrder === 'asc' ? sortCategoriesAsc(entry.categories) : sortCategoriesDesc(entry.categories);
              ...
              >
                <div className="flex flex-col items-center justify-center gap-1 w-[76px] sm:w-[84px] shrink-0 border-r border-gray-200/60 dark:border-gray-700/60 pr-2">
                  <BankLogo 
                    ...
                    size="md"
                  />
                  <span className="text-[8px] sm:text-[9px] font-bold text-center text-gray-700 dark:text-gray-300 uppercase tracking-tight leading-tight w-full break-words">{formatBankName(bank.name)}</span>
                </div>
                
                <div className="flex flex-col justify-center flex-1 py-0.5">
                  <div className="flex flex-col bg-white dark:bg-gray-800 border border-gray-200/60 dark:border-gray-700/60 rounded shadow-sm overflow-hidden">
                      ...
                      return (
                        <div 
                          ...
                          className={clsx(
                            ...
                            idx !== arr.length - 1 && "border-b border-gray-100 dark:border-gray-700/50"
                          )}
                        >
                          <span className="leading-none break-words">{formatCategoryName(name)}</span>
                          {percent && (
