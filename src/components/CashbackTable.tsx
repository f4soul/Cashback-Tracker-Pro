...
export const CashbackTable: React.FC<CashbackTableProps> = memo(({ 
  ...
  const sortedEntries = useMemo(() => {
    return entries.map(entry => {
      const sortedCategories = sortOrder === 'asc' 
              ...
              <>
                ...
                <h2 className="hidden [.pdf-export-mode_&]:block text-lg font-bold text-gray-900 dark:text-white tracking-tight whitespace-nowrap">
                  <span className="text-[var(--accent-color)]">
                    {capitalize(formatMonthId(selectedMonthId || monthId).split(' ')[0])}
                  </span>
                </h2>
              </>
            ) : (
                      ...
                      return (
                        <div 
                          ...
                          className={clsx(
                            ...
                            idx !== arr.length - 1 && "border-b border-gray-100 dark:border-gray-700/60"
                          )}
                        >
                          <span className="leading-tight truncate pr-4 flex-1">{formatCategoryName(name)}</span>
                          {percent && (
