...
import { getCurrentMonthId, getNextMonthId, getPreviousMonthId, formatMonthId, capitalize } from '../utils/date';
...
const BankCard = React.forwardRef<HTMLDivElement, BankCardProps>(({
  entry, bank, logoShape, onEdit, onDelete, isDragging, isOverlay, attributes, listeners, style
}, ref) => {
  return (
    <div
      ref={ref}
      ...
      className={clsx(
        ...
        isOverlay && "border-[var(--accent-color)] shadow-2xl scale-105 z-50 ring-2 ring-[var(--accent-color)] ring-opacity-50 cursor-grabbing bg-white dark:bg-gray-800"
      )}
    >
...
const SortableBankCard: React.FC<SortableBankCardProps> = memo(({ entry, bank, logoShape, onEdit, onDelete }) => {
  ...
  const style = {
    ...
    transition: transition || (isDragging ? undefined : 'transform 250ms cubic-bezier(0.32, 0.72, 0, 1)'),
            ...
            >
              {data.entries.length === 0 ? (
                  ...
                  >
                    ...
                    >
                      {activeDragId ? (() => {
                        const entry = data.entries.find(e => e.id === activeDragId);
