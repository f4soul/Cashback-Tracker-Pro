...
export function useThemeSync(activeTab: 'current' | 'archive' | 'settings') {
  ...
  useEffect(() => {
    ...
    if (isDark) {
      root.style.setProperty('--percent-bg', `color-mix(in srgb, ${settings.percentBlockText} 12%, transparent)`);
      root.style.setProperty('--percent-text', `color-mix(in srgb, ${settings.percentBlockText} 90%, #ffffff)`);
    } else {
